...
# Substitute: matrix-games, cogame-matrix-games, 8.
#
...
#
# env:
#   SMOKE_IMAGE                image, if not given as $1        (cogame-matrix-games:ci)
#   SMOKE_SLUG                 game slug                        (matrix-games)
#   SMOKE_GAME_BIN             game entrypoint                  (/bin/matrix-games)
#   SMOKE_PLAYER_BIN           player entrypoint                (/bin/matrix-games-player)
...
image="${1:-${SMOKE_IMAGE:-cogame-matrix-games:ci}}"
slug="${SMOKE_SLUG:-matrix-games}"
...
# (cogame-raid 0.1.3 -> 0.1.4, 2026-08-23: the starter smoke only checked the
# game's exit code, so a player that raised on a close frame passed one
# dispatch and failed the next.)
for ((slot = 0; slot < seats; slot++)); do
